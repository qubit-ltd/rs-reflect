...
`AdapterTypeMismatch` 和 `Found`。便捷的 `capability` 方法将 `Found` 映射为
`Ok(Some(adapter))`，将 `Missing` 映射为 `Ok(None)`；`FactOnly` 和
`AdapterTypeMismatch` 保留为错误。无效的 intrinsic 能力集合返回冲突错误。
有效事实还会保留 `CapabilityOrigin::Intrinsic { type_id }` 或
