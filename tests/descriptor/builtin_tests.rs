...
static LAZY_OPTIONAL_ELEMENT_DESCRIPTOR: TypeDescriptor =
    reflect::__private::codegen_v3::descriptor::struct_type::<LazyOptionalElement>(
...
static LAZY_BOX_POINTEE_DESCRIPTOR: TypeDescriptor =
    reflect::__private::codegen_v3::descriptor::struct_type::<LazyBoxPointee>(
        "lazy_box_pointee",
        StructKind::Unit,
        &[],
    );
...
macro_rules! define_lazy_probe {
    ($type_name:ident, $counter:ident, $descriptor:ident, $query_name:literal) => {
        ...
        static $descriptor: TypeDescriptor =
            reflect::__private::codegen_v3::descriptor::struct_type::<$type_name>(
                $query_name,
                StructKind::Unit,
                &[],
            );
...
fn test_builtin_tuple_and_function_arity_boundaries() {
    for (descriptor, expected) in [
        ...
        assert_eq!(
            descriptor.as_tuple().expect("tuple descriptor").arity(),
            expected
        );
    }
    for (descriptor, expected) in [
