...
fn test_trait_descriptor_public_paths_remain_available() {
    ...
    let cached =
        reflect::descriptor::cached_trait_object_descriptor::<PublicPathObjectMarker>(|| {
            TraitDescriptor::builder(&ROOT_DEFINITION)
                .build()
...
fn test_trait_payload_and_object_caches_reuse_initialized_values() {
    ...
    let first = reflect::__private::codegen_v3::descriptor::cached_trait_object_descriptor::<
    ...
    let second = reflect::__private::codegen_v3::descriptor::cached_trait_object_descriptor::<
        TraitObjectCacheMarker,
...
fn test_trait_descriptor_inspection_apis_preserve_local_facts() {
    ...
    let thread_safe_adapter =
        InvocationAdapter::thread_safe_with_unavailable_catching(thread_safe_invocation_entry);
    assert!(
        thread_safe_adapter
            .invoke_thread_safe(
                ReflectRegistry::initialize().expect("valid fixture registry"),
    ...
    let associated_type =
        AssociatedTypeDescriptor::new(3, "Item", "item", Box::new([]), Some(TypeExpression::Never));
    ...
    let associated_const =
        AssociatedConstDescriptor::new(4, "LIMIT", "limit", TypeExpression::Never, true);
