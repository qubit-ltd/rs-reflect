...
fn default_method<'a>(
    ...
    let implementations = registry
        .implementations(DefaultInvocationSample::type_descriptor().type_id());
...
fn test_default_trait_adapter_supports_safe_owned_non_path_outputs_and_never() {
    ...
    let function =
        invoke_default_owned(registry, "default_function_pointer_output");
...
fn test_default_trait_adapter_preserves_the_dedicated_str_variant_and_origin() {
    ...
    let output = default_method(registry, "default_shared_str")
        ...
        .invoke_local(Invocation::associated([
            reflect::invoke::InvocationArg::Ref(reflect::value::DynamicRef::<
                reflect::value::Local,
            >::new_str(
                "default"
            )),
    ...
    let cases: [(&str, DynamicOwned<reflect::value::Local>, u8); 4] = [
        (
            ...
            DynamicOwned::<reflect::value::Local>::new(Box::new(
                DefaultInvocationSample,
            )),
            29,
        ),
        (
            "default_rc_receiver",
            DynamicOwned::<reflect::value::Local>::new(Rc::new(
                DefaultInvocationSample,
            )),
            31,
        ),
...
fn test_default_trait_adapter_supports_pinned_borrow_receivers() {
    let registry = ReflectRegistry::initialize()
        .expect("generated impl fragments must validate");
    let shared_instance =
        default_method(registry, "default_pinned_ref_receiver");
    ...
    let mutable_instance =
        default_method(registry, "default_pinned_mut_receiver");
...
fn test_default_trait_adapter_supports_thread_safe_mode() {
    ...
    let output = adapter
        .invoke_thread_safe(Invocation::associated([
            reflect::invoke::InvocationArg::Owned(
...
fn test_positional_patterns_are_not_invocation_blockers() {
    let registry = ReflectRegistry::initialize()
        .expect("generated impl fragments must validate");
    let implementations = registry
        .implementations(PatternReasonSample::type_descriptor().type_id());
