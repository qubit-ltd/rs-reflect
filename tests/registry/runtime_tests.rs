...
static EARLY_DESCRIPTOR: TypeDescriptor =
    reflect::__private::codegen_v3::descriptor::opaque_root::<EarlyType>(
        "shared-query",
    );
static LATE_DESCRIPTOR: TypeDescriptor =
    reflect::__private::codegen_v3::descriptor::opaque_root::<LateType>(
        "shared-query",
    );
static INDEPENDENT_DESCRIPTOR: TypeDescriptor =
    reflect::__private::codegen_v3::descriptor::opaque_root::<IndependentType>(
        "independent",
    );
...
fn capability_runtime_identity() -> RuntimeIdentity {
    RuntimeIdentity::Capabilities(RegistrationCapabilityTarget::Type(
...
static EMPTY_GENERIC_DEFINITION: LazyLock<GenericDefinitionDescriptor> =
    LazyLock::new(|| {
        GenericDefinitionDescriptor::new(Box::default(), Box::default())
    });
