...
fn main() {
    let registry =
        registry::ReflectRegistry::initialize().expect("generated fragments must validate");
    let implementations = registry.implementations(Worker::type_descriptor().type_id());
    let descriptor::MethodLookup::Unique(method) = descriptor::ImplDescriptor::lookup_method(
        implementations,
