...
fn concrete_identity() -> RuntimeIdentity {
    RuntimeIdentity::Capabilities(CapabilityTarget::Type(TypeId::of::<
        ConcreteTarget,
    >()))
}
...
fn intrinsic_capabilities()
-> Result<&'static TypeCapabilities, CapabilityConflict> {
    static RESULT: OnceLock<Result<TypeCapabilities, CapabilityConflict>> =
        OnceLock::new();
    RESULT
        .get_or_init(|| {
...
impl Reflect for IntrinsicTarget {
    fn type_descriptor() -> &'static TypeDescriptor {
        static DESCRIPTOR: TypeDescriptor =
            opaque_root::<IntrinsicTarget>("IntrinsicTarget")
...
fn test_cross_fragment_mismatch_retains_complete_ordered_diagnostics() {
    let error = build_registry(&[&MISMATCH_RIGHT, &MISMATCH_LEFT])
        .expect_err("conflict");
...
fn test_cross_fragment_duplicate_and_fact_conflicts_share_classification() {
    for (fragments, id) in [
        ...
        let error =
            build_registry(&fragments).expect_err("duplicate capability ID");
        let detail =
            error.capability_details().expect("full capability context");
        let capability_id = error.capability_id().expect("capability ID");
...
fn test_definition_target_is_reported_with_both_contracts() {
    let error = build_registry(&[&DEFINITION_RIGHT, &DEFINITION_LEFT])
...
fn test_intrinsic_conflict_retains_target_source_and_error_chain() {
    let error =
        build_registry(&[&INTRINSIC_FRAGMENT]).expect_err("intrinsic conflict");
    ...
    let intrinsic_source = FragmentIdentity::new(
        "intrinsic-source",
        "diagnostics",
        90,
        1,
...
fn test_source_order_normalizes_complete_diagnostics() {
    let forward = build_registry(&[&MISMATCH_LEFT, &MISMATCH_RIGHT])
        .expect_err("conflict");
    let reverse = build_registry(&[&MISMATCH_RIGHT, &MISMATCH_LEFT])
        .expect_err("conflict");
...
fn test_legacy_constructors_remain_usable_without_inventing_context() {
    ...
    let intrinsic = RegistryError::intrinsic_capability_conflict(
        source("legacy", 120),
        conflict.clone(),
    );
