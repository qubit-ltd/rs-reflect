...
static REGISTRY_BENCHMARK_DESCRIPTOR: TypeDescriptor =
    reflect::__private::descriptor::opaque_root::<RegistryBenchmarkType>(
        "registry-benchmark",
    );
...
static REGISTRY_BENCHMARK_FRAGMENT: RegistrationFragment =
...
fn main() {
    ...
    measure_repeated(
        "post-materialization aggregation: 1 fragment",
        1_000,
        || aggregate_benchmark_registry_facts(&one_fragment),
    );
    measure_repeated(
        "post-materialization aggregation: 100 fragments",
        100,
        || aggregate_benchmark_registry_facts(&one_hundred_fragments),
    );
    measure_repeated(
        "post-materialization aggregation: 10000 fragments",
        3,
        || aggregate_benchmark_registry_facts(&ten_thousand_fragments),
    );

    let registry = ReflectRegistry::initialize()
        .expect("production registry initialization");
    ...
    for batch_size in [1_usize, 100, 10_000] {
        measure_repeated(
            &format!("frozen lookup batch {batch_size}"),
            batch_size,
            || {
                (
