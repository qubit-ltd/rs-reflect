...
pub(in crate::expand::impls) fn substitute_type_syntax(
    ...
    let mut ty: Type = parse2(tokens.clone()).expect(
        "validated specialization target must remain valid type syntax",
    );
    ...
    fn visit_type_mut(&mut self, ty: &mut Type) {
        if let Type::Path(path) = ty
            ...
            && let Some(replacement) =
                self.replacement(&path.path.segments[0].ident)
        {
            if path.path.segments.len() > 1 {
                ...
                if let Ok(mut replacement) =
                    parse2::<Type>(quote!(#replacement :: #tail))
                {
    ...
    fn visit_expr_mut(&mut self, expression: &mut Expr) {
                ...
                self.replacement(&path.path.segments[0].ident)
        {
            if path.path.segments.len() > 1 {
                ...
                if let Ok(mut replacement) =
                    parse2::<Expr>(quote!(#replacement :: #tail))
                {
...
mod tests {
    ...
    fn test_substitute_path_syntax_rewrites_trait_type_and_const_arguments() {
        let input: Path = parse2(quote!(Trait<T, N>))
            .expect("the trait path must be valid path syntax");
    ...
    fn test_substitute_type_syntax_preserves_absolute_paths() {
        ...
        for (input, expected) in cases {
            let actual: Type =
                parse2(substitute_type_syntax(&input, &replacements()))
                    .expect("substitution must retain valid type syntax");
            let expected: Type = parse2(expected)
    ...
    fn test_substitute_type_syntax_rewrites_qself_and_const_expressions() {
        ...
        let expected: Type =
            parse2(quote!([<Vec<u8> as Trait<4>>::Assoc; 4 + 1]))
                .expect("the expected result must be valid type syntax");
