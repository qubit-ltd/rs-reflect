...
let methods: Vec<_> = declaration
    ...
    .map(|(method_index, method)| {
        ...
        let receiver = match &method.receiver {
            Some(receiver) => match receiver.kind {
                ...
                ReceiverKindIr::Typed => {
                    ...
                    quote!(Some(#facade::__private::codegen_v1::descriptor::ReceiverDescriptor::Explicit(#declaration)))
                }
            },
            None => quote!(None),
        };
        let parameters = method.parameters.iter().map(|parameter| {
            ...
            quote!(#facade::__private::codegen_v1::descriptor::ParameterDescriptor::new(#index, #name, #pattern, #passing, #ty, None))
        });
        ...
        let abi = match abi {
            Some(abi) => quote!(Some(#facade::__private::codegen_v1::expression::FunctionAbi::Other(#abi.into()))),
            None => quote!(None),
        };
        quote! {
            #facade::__private::codegen_v1::descriptor::MethodDescriptor::builder(
                ...
                #name,
                #query,
                #facade::__private::codegen_v1::descriptor::MethodDeclarationOwner::Trait(definition),
            )
...
let associated_types: Vec<_> = declaration.associated_types.iter().enumerate().map(|(index, item)| {
    ...
    let bounds = item.bounds.iter().filter_map(|bound| match bound {
        GenericBoundIr::Trait { path, lifetimes, modifier } => {
            ...
            let modifier = match modifier { crate::ir::TraitBoundModifierIr::None => quote!(#facade::__private::codegen_v1::expression::TraitBoundModifier::None), crate::ir::TraitBoundModifierIr::Maybe => quote!(#facade::__private::codegen_v1::expression::TraitBoundModifier::Maybe) };
    ...
    quote!(#facade::__private::codegen_v1::descriptor::AssociatedTypeDescriptor::new_with_generic_definition(
        #index,
        #name,
        #name,
        Box::new([#(#bounds),*]),
...
let associated_consts: Vec<_> = declaration
    ...
    .map(|(index, item)| {
        ...
        quote!(#facade::__private::codegen_v1::descriptor::AssociatedConstDescriptor::new(#index, #name, #name, #ty, #has_default))
    })
    .collect();
