...
pub(super) fn default_method_invocation_adapter(
    ...
    let receiver_expectation = if matches!(
        ...
        quote!(#facade::__private::codegen_v3::invoke::ReceiverExpectation::owned::<Self>())
        ...
        match method.receiver.as_ref().map(|receiver| receiver.kind) {
            ...
            Some(ReceiverKindIr::MutableReference) => {
                quote!(#facade::__private::codegen_v3::invoke::ReceiverExpectation::borrowed_mut::<Self>())
            }
            Some(ReceiverKindIr::SharedReference) => {
                quote!(#facade::__private::codegen_v3::invoke::ReceiverExpectation::borrowed::<Self>())
            }
            Some(ReceiverKindIr::Typed) => return None,
            None => quote!(#facade::__private::codegen_v3::invoke::ReceiverExpectation::none()),
        }
    };
        ...
        Some(ReceiverKindIr::Value) => quote! {
            ...
            let receiver: Self = match receiver {
                Some(#facade::__private::codegen_v3::invoke::InvocationReceiver::Owned(value)) =>
                    #facade::__private::codegen_v3::value::DynamicOwned::<#mode>::downcast::<Self>(value)
        ...
        Some(ReceiverKindIr::MutableReference) => quote! {
            ...
            let receiver: &mut Self = match receiver {
                Some(#facade::__private::codegen_v3::invoke::InvocationReceiver::Mut(value)) =>
                    #facade::__private::codegen_v3::value::DynamicMut::<#mode>::downcast::<Self>(value)
                        .unwrap_or_else(|_| unreachable!("validation checked receiver type")),
                _ => unreachable!("validation checked receiver mode"),
            };
        },
        Some(ReceiverKindIr::SharedReference) => quote! {
            ...
            let receiver: &Self = match receiver {
                Some(#facade::__private::codegen_v3::invoke::InvocationReceiver::Ref(value)) =>
                    #facade::__private::codegen_v3::value::DynamicRef::<#mode>::downcast::<Self>(value)
                        .unwrap_or_else(|_| unreachable!("validation checked receiver type")),
                Some(#facade::__private::codegen_v3::invoke::InvocationReceiver::Mut(value)) => {
                    let value = #facade::__private::codegen_v3::value::DynamicMut::<#mode>::downcast::<Self>(value)
                        .unwrap_or_else(|_| unreachable!("validation checked receiver type"));
                    &*value
                }
                _ => unreachable!("validation checked receiver mode"),
    ...
    let adapter_constructor = if catching_requested && thread_safe {
        ...
        quote!(#facade::__private::codegen_v3::descriptor::InvocationAdapter::thread_safe(Self::#adapter_name))
    } else {
        quote!(#facade::__private::codegen_v3::descriptor::InvocationAdapter::local(Self::#adapter_name))
    };
    ...
    let adapter_entry = if catching_requested {
        quote! {{
            ...
            Some(::std::boxed::Box::leak(::std::boxed::Box::new(adapter))
                as &'static #facade::__private::codegen_v3::descriptor::InvocationAdapter)
        }}
    } else {
        quote! {
...
fn default_pinned_method_invocation_adapter(
    ...
    let invocation_type = if pinned_mutable {
        ...
        quote!(#facade::__private::codegen_v3::invoke::PinnedRefInvocation<'call, Self, #mode>)
    };
    let failure_type = if pinned_mutable {
        quote!(#facade::__private::codegen_v3::invoke::PinnedMutInvocationFailure<'call, Self, #mode>)
    } else {
        quote!(#facade::__private::codegen_v3::invoke::PinnedRefInvocationFailure<'call, Self, #mode>)
    };
    let adapter_type = if pinned_mutable {
        quote!(#facade::__private::codegen_v3::invoke::PinnedMutAdapter<Self, #mode>)
    } else {
        quote!(#facade::__private::codegen_v3::invoke::PinnedRefAdapter<Self, #mode>)
    };
    ...
    let constructor = if pinned_mutable {
        quote!(#facade::__private::codegen_v3::descriptor::InvocationAdapter::pinned_mut_local(adapter))
    } else {
        quote!(#facade::__private::codegen_v3::descriptor::InvocationAdapter::pinned_ref_local(adapter))
    };
