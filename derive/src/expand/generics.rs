        ...
        declaration.generics.params.iter().enumerate()
    {
        match parameter.kind {
            ...
            GenericKindIr::Const => {
                ...
                let name = Ident::new(&parameter.name, parameter.span);
                let declared_type = super::traits::type_expression(
                    const_type,
                    &environment,
                    facade,
                );
...
pub(crate) fn type_definition_provider_name(
    declaration: &TypeDeclarationIr,
) -> Ident {
    for attribute in &declaration.attributes {
        if let crate::ir::HelperValueIr::DefinitionProviderV2(name) =
            &attribute.value
        {
            return name.clone();
        }
...
pub(crate) fn type_definition_provider(
    ...
    let marker =
        format_ident!("__QubitReflectTypeDefinition{}", declaration.name);
    let registration_module = format_ident!(
        "__qubit_reflect_type_definition_registration_{fingerprint:016x}"
    );
    let environment = GenericEnvironment::from_generics(&declaration.generics);
...
fn symbolic_visibility(
    visibility: &crate::ir::VisibilityIr,
    facade: &TokenStream,
) -> TokenStream {
    match visibility {
...
mod tests {
    ...
    fn test_transparent_reflect_bounds_only_follow_known_container_paths() {
        ...
        let parameters: Vec<_> =
            transparently_reflected_type_parameters(&declaration)
                .into_iter()
