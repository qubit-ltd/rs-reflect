...
fn convert_meta(
    ...
    let value = match name {
        HelperName::DefinitionProviderV2 => match &meta {
            Meta::NameValue(value) => parse2(value.value.to_token_stream())
                .map(HelperValueIr::DefinitionProviderV2),
            _ => Err(Error::new(
                span,
                "definition_provider_v2 requires one function identifier",
            )),
        },
...
fn parse_specialization(meta: &Meta) -> SynResult<SpecializationIr> {
    ...
    let SpecializationBindings(bindings) =
        parse2::<SpecializationBindings>(list.tokens.clone())?;
