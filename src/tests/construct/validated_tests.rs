...
static UPDATE_FIELDS: [UpdateField; 1] =
    [UpdateField::allowed(&FIXTURE_FIELDS[0])];
...
fn test_validated_unit_input_inspection_supports_both_modes() {
    let local = validate_unit::<Local>(&[])
        .expect("an empty unit contract must validate");
    assert!(local.values().is_empty());
    assert!(format!("{local:?}").contains("value_count: 0"));
    assert!(local.into_values().is_empty());

    let thread_safe = validate_unit::<ThreadSafe>(&[])
        .expect("an empty unit contract must validate");
...
fn test_validated_update_input_inspection_supports_both_modes() {
    let local_input = StructUpdateInput::new(
        ...
        NamedConstructionInput::<Local>::new(
            Vec::<(&str, DynamicOwned<Local>)>::new(),
        ),
    );
    ...
    let local =
        validate_update(local_input, TypeId::of::<u8>(), &[], local_type_id)
    ...
    let thread_safe_input = StructUpdateInput::new(
        ...
        NamedConstructionInput::<ThreadSafe>::new(Vec::<(
            &str,
...
fn test_validated_override_inspection_supports_both_modes() {
    ...
    assert!(
        format!("{:?}", local.overrides()[0]).contains("ValidatedOverride")
    );
    ...
    let thread_safe = validate_update(
        StructUpdateInput::new(
            ...
            NamedConstructionInput::new([(
                "renamed",
                DynamicOwned::<ThreadSafe>::new(10_u8),
            )]),
        ),
        TypeId::of::<u32>(),
        &UPDATE_FIELDS,
    ...
    assert!(
        format!("{:?}", thread_safe.overrides()[0])
            .contains("ValidatedOverride")
    );
