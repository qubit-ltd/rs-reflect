...
static UNIT_DEFINITION: LazyLock<TraitDefinitionDescriptor> =
    LazyLock::new(|| {
...
static CHILD_DEFINITION: LazyLock<TraitDefinitionDescriptor> =
    LazyLock::new(|| {
...
static ALTERNATE_DEFINITION: LazyLock<TraitDefinitionDescriptor> =
    LazyLock::new(|| {
        TraitDefinitionDescriptor::new(
...
fn test_trait_descriptor_views_expose_all_local_facts() {
    ...
    let root = Box::leak(Box::new(
        TraitDescriptor::builder(&UNIT_DEFINITION)
            .associated_types(vec![
                UNIT_DEFINITION.associated_types()[0].clone(),
            ])
            .associated_consts(vec![
                UNIT_DEFINITION.associated_consts()[0].clone(),
            ])
            .build()
    ...
    let associated_type =
        root.associated_type("item").expect("associated type");
    ...
    let associated_const =
        root.associated_const("limit").expect("associated const");
