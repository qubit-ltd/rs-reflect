...
use crate::__private::codegen_v3::expression as codegen_expression;
...
#[test]
fn test_codegen_v3_expression_factories_preserve_structural_inputs() {
    ...
    let reference = codegen_expression::reference(
...
fn test_descriptor_factories_preserve_typed_categories() {
    ...
    let trait_object =
        descriptor::trait_object::<dyn std::fmt::Debug>("debug", debug_trait_descriptor);
    let opaque = descriptor::opaque_root::<()>("opaque");
    let opaque_caps =
        descriptor::opaque_root_with_capabilities::<()>("opaque-caps", empty_capabilities);
