...
pub type FieldGetMutAdapter =
    for<'a> fn(
        DynamicMut<'a, Local>,
    ) -> Result<DynamicMut<'a, Local>, FieldAccessError>;

/// A whole-value replacement adapter for a field.
pub type FieldSetAdapter = for<'a> fn(
    DynamicMut<'a, Local>,
    DynamicOwned<Local>,
...
#[doc(hidden)]
pub type FieldSetPreflightAdapter =
    for<'a> fn(&DynamicMut<'a, Local>) -> Result<(), FieldAccessError>;
...
pub type ThreadSafeFieldSetAdapter = for<'a> fn(
    DynamicMut<'a, ThreadSafe>,
...
#[doc(hidden)]
pub type ThreadSafeFieldSetPreflightAdapter =
    for<'a> fn(&DynamicMut<'a, ThreadSafe>) -> Result<(), FieldAccessError>;
...
pub(crate) fn thread_safe_ref_type_id(
    value: &DynamicRef<'_, ThreadSafe>,
) -> TypeId {
    value.as_any().map_or_else(TypeId::of::<str>, |value| {
        (value as &dyn std::any::Any).type_id()
    })
}

/// Returns the exact identity carried by a thread-safe mutable value.
pub(crate) fn thread_safe_mut_type_id(
    value: &DynamicMut<'_, ThreadSafe>,
) -> TypeId {
    value.as_any().map_or_else(TypeId::of::<str>, |value| {
        (value as &dyn std::any::Any).type_id()
    })
...
pub(crate) fn thread_safe_owned_type_id(
    value: &DynamicOwned<ThreadSafe>,
) -> TypeId {
    (value
        .as_any()
        .expect("thread-safe owned dynamic values are always Any-compatible")
        as &dyn std::any::Any)
        .type_id()
}
